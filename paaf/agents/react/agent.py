...
"""
    ...
    async def arun(self, query: str) -> AgentResponse:
        ...
        try:
            ...
            if self.step_callback and self.execution_summary.steps:
                ...
                await self._asend_callback(callback_data)
    ...
    def _start(self) -> AgentResponse:
        ...
        if self.output_format is not None:
            ...
            try:
                ...
                self.log(f"Error formatting output: {e}", "error")
                final_response = last_message.content
        else:
            final_response = last_message.content
    ...
    async def _astart(self) -> AgentResponse:
        """
        Async version of _start method.
        """
        if self.query is None:
            raise ValueError("Query must be provided before starting the agent.")
    ...
    def think(self):
        ...
        self.log(f"Iteration {self.current_iteration}: {response}", "info")
    ...
    async def _athink(self):
        ...
        if self.current_iteration > self.max_iterations:
            error_msg = f"Maximum number of iterations ({self.max_iterations}) reached."
            think_step.error = error_msg
            await self._asend_callback(think_step)
            raise ValueError(error_msg)
        ...
        return await self._adecide_action(response)
    ...
    async def _adecide_action(self, response: ReactAgentResponse):
        """
        Async version of decide_action method.
        """
        if response.action_type == ReactAgentActionType.TOOL_CALL:
            # If the action type is TOOL_CALL, we need to choose a tool and its arguments
            ...
            await self._aact(response.tool_choice, tool_arguments)
            ...
            # Send callback for final answer
            await self._asend_callback(answer_step)
            ...
            if not response.handoff:
                ...
                handoff_step.error = error_msg
                await self._asend_callback(handoff_step)
                raise ValueError(error_msg)
    ...
    async def _aact(self, tool_choice: ToolChoice, tool_arguments: dict):
        ...
        if tool_choice.tool_id not in self.tools_registry.tools:
            error_msg = f"Tool {tool_choice.name} not found in registry."
            act_step.error = error_msg
            await self._asend_callback(act_step)
            raise ValueError(error_msg)
        ...
        try:
            ...
            result = await self.tools_registry.acall_tool(
                tool_choice.tool_id, **tool_arguments
            )
            act_step.tool_result = result
            ...
            await self._asend_callback(observe_step)
    ...
    async def _asend_callback(self, step_summary: ReactStepSummary):
        """Async version of send callback."""
        if not self.step_callback:
            return
        ...
        try:
            if inspect.iscoroutinefunction(self.step_callback):
                await self.step_callback(callback_data)
            else:
                self.step_callback(callback_data)
        except Exception as e:
            self.log(f"Error in step callback: {e}", "error")
